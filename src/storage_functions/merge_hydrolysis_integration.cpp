...
int merge_hydrolysis_files_integration(
	...
	std::string output_names)
{
	...
	std::istringstream output(output_names); //EDIT PAUL
	float y = x + timestep; //EDIT PAUL
	
    // Dateien, die eingelesen werden sollen:
    string file_name;
    ifstream file_output1;
    ifstream file_output2;

    // Fehler abfangen: Falls Dateien nicht vorhanden sind
    if(!file_output1){
        cout << "File not found (output1)." << std::endl;
        return 0;
    }
    else if(!file_output2){
        cout << "File not found (output2)." << std::endl;
        return 0;
    }
    ...
    for (int runde=1; runde<anzahl; runde++){
        ...
        if (runde == 1){
            ...
            file_output1 = ifstream (file_name);
            getline(output, file_name);
            ifstream file_output2(file_name);
        }
        else{
            file_output2 = ifstream (output_file_name); //EDIT PAUL
            getline(output,file_name);
            file_output1 = ifstream (file_name);
        }
            if(!file_output1){
				cout << "file_name: " << file_name << std::endl;
                cout << "File not found (output1). HERE" << std::endl;
                return 0;
            }
            else if(!file_output2){
                cout << "File not found (output2)." << std::endl;
            ...
            while(file_output1 >> zahl1)  //Zahlenweise durchgehen
            {
                ...
                //Fehler abfangen: kein Wert als Eingabe
                    ...
                    cout << "falscher Wert in 1.Outflow-Datei (keine Zahl)" << endl;
                    return 0;
                }
            }
            ...
            while(file_output2 >> zahl2)  //Zahlenweise durchgehen
            {
                ...
                //Fehler abfangen: kein Wert als Eingabe
                    ...
                    cout << "falscher Wert in 2.Outflow-Datei (keine Zahl)" << endl;
                    return 0;
                }
            }
            ...
            if (endung == 0){
                cout << "Einheiten-Angabe fehlt in Eingabedatei" << endl;
                return 0;
            }
                ...
                while ((i1<liste1.size()) && (wert1 <aktueller_timestamp1)){ //solange noch Elemente vorhanden sind und der aktuelle Zeitstempel nicht erreicht wurde
                    for (int j1=0; j1<z; ++j1){
                        if (i1!=0 && j1==0){ //jeder Zeitpunkt (außer erster)
                            ...
                            if (wert1<wert2){
                                cout << "Zeitangabe in 1.Outflow-Datei falsch: Zeit darf nicht kleiner werden" << endl;
                                return 0;
                            }
                        ...
                        //////////////////////////////////////////////////////////////////////
                            ...
                            testString += "wert3: " + to_string(wert3) + 
                            " wert4: " + to_string(wert4) + 
                            " wert5:" + to_string(wert5) + "\n";
                ...
                while (i2<liste2.size()&& (wert1_2 <aktueller_timestamp2)){
                    for (int j2=0; j2<z; ++j2){
                        if ((i2!=0 && j2==0)){ //jeder Zeitpunkt (außer erster)
                            ...
                            //Fehlerbehandlung: Zeit wird wieder kleiner
                            if (wert1_2<wert2_2){
                                cout << "Zeitangabe in 2.Outflow-Datei falsch: Zeit darf nicht kleiner werden" << endl;
                                return 0;
                            }
                            ...
                            testString += "wert3_2: " + to_string(wert3_2) + 
                            " wert4_2: " + to_string(wert4_2) + 
                            " wert4_2_b:" + to_string(wert4_2_b) + 
                            " wert5_2:" + to_string(wert5_2) + "\n";
            ...
            ofstream gesamtdatei(output_file_name, ios::out);
            string gesamtdatei_string = "";
            ...
            gesamtdatei << gesamtdatei_string;
    }
    
    // Sonderfall: ein Reaktor (keine Veränderungen der Eingabedatei)
    if (anzahl == 1){
        getline(output,file_name);
        string outputdatei = output_file_name;
        const char* char_type = outputdatei.c_str();
        const char* file_name1 = file_name.c_str();
        rename(file_name1, char_type);
    }
    return 0;
}
